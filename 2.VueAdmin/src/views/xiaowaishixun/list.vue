	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('xiaowaishixun','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('xiaowaishixun','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('xiaowaishixun','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('xiaowaishixun','删除')">删除</el-button>
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('xiaowaishixun','已打卡统计')">已打卡统计</el-button>
					<el-button type="warning" @click="echartClick2" v-if="btnAuth('xiaowaishixun','未打卡统计')">未打卡统计</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('xiaowaishixun','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'已打卡统计'" width="70%">
			<el-tabs v-model="echartActive" class="demo-tabs" @tab-change="echartTabClick" type="card">
			    <el-tab-pane label="已打卡统计" name="1"></el-tab-pane>
			    <el-tab-pane label="未打卡统计" name="2"></el-tab-pane>
			</el-tabs>
			<div v-if="echartActive==1" id="yidakaEchart1" style="width:100%;height:600px;"></div>

			<div v-if="echartActive==2" id="weidakaEchart2" style="width:100%;height:600px;"></div>
...
<script setup>
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var yidakaEchart1 = echarts.init(document.getElementById("yidakaEchart1"),'macarons');
			context?.$http({
				url: `xiaowaishixun/value/xueshengxingming/yidaka`,
				method: 'get'
			}).then(res=>{
				...
				// 使用刚指定的配置项和数据显示图表。
				yidakaEchart1.setOption(option);
				  //根据窗口的大小变动图表
				window.onresize = function() {
				    yidakaEchart1.resize();
				};
			})
		})
	}
	...
	const echartClick2 = ()=>{
		...
		echartVisible.value = true
		nextTick(()=>{
			var weidakaEchart2 = echarts.init(document.getElementById("weidakaEchart2"),'macarons');
			context?.$http({
				url: `xiaowaishixun/value/xueshengxingming/weidaka`,
				method: 'get'
			}).then(res=>{
				let obj = res.data.data
				let xAxis = [];
				...
				// 使用刚指定的配置项和数据显示图表。
				weidakaEchart2.setOption(option);
				  //根据窗口的大小变动图表
				window.onresize = function() {
				    weidakaEchart2.resize();
				};
			})
		})
	}
