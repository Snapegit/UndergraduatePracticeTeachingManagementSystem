	...
	<div>
		<div class="app-contain">
			<div class="list_search_view">
				...
				<br>
				<div class="btn_view">
					<el-button type="success" @click="addClick" v-if="btnAuth('xiaoneishixun','新增')">新增</el-button>
					<el-button  v-if=" btnAuth('xiaoneishixun','查看')" type="info"  :disabled="selRows.length==1?false:true" @click="infoClick(null)">详情</el-button>
					<el-button type="primary" :disabled="selRows.length==1?false:true" @click="editClick" v-if=" btnAuth('xiaoneishixun','修改')">修改</el-button>
					<el-button type="danger" :disabled="selRows.length?false:true" @click="delClick(null)"  v-if="btnAuth('xiaoneishixun','删除')">删除</el-button>
					<el-button type="warning" @click="echartClick1" v-if="btnAuth('xiaoneishixun','参与情况统计')">参与情况统计</el-button>
				</div>
			</div>
			<br>
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='true' align="left" header-align="left">
					<template #default="scope">
						<el-button type="info" v-if=" btnAuth('xiaoneishixun','查看')" @click="infoClick(scope.row.id)">详情</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
		...
		<!-- 统计图弹窗 -->
		<el-dialog v-model="echartVisible" :title="'参与情况统计'" width="70%">
			<div  id="canyuqingkuangEchart1" style="width:100%;height:600px;"></div>
...
<script setup>
	...
	const echartClick1 = ()=>{
		...
		nextTick(()=>{
			var canyuqingkuangEchart1 = echarts.init(document.getElementById("canyuqingkuangEchart1"),'macarons');
			context?.$http({
				url: `xiaoneishixun/value/xueshengxingming/canyuqingkuang`,
				method: 'get'
			}).then(res=>{
				...
				// 使用刚指定的配置项和数据显示图表。
				canyuqingkuangEchart1.setOption(option);
				  //根据窗口的大小变动图表
				window.onresize = function() {
				    canyuqingkuangEchart1.resize();
				};
			})
		})
	}
